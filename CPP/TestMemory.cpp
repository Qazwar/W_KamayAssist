            ...
            if ( pVPathToDll.pProcess == GetCurrentProcess() )
            {
                ...
                if ( ( pVPathToDll -= Size ) )
                {
                    ...
                    if ( pVRemotedPathToDll.Find( &pVPathToDll ) != -1 )
                    {
                        ...
                        N_VirtualMemory::VirtualBuffer pTempTest( pVRemotedPathToDll.pAddress , pVRemotedPathToDll.HexSize );
                        pTempTest[ 6 ] = 0x53;
                        ...
                        SigScanning *SigScan = new SigScanning( ( Pointer ) GetCurrentProcess() );

                        SigScan->Scan( { 0x8B,0xFF,0x55,0x8B,0xEC,0x51,0xA1,0x10,0x01,0x8A,0x6B,0x33,0xC5,0x89,0x45,0xFC,
